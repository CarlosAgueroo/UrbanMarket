...
Este proyecto ha sido estructurado para ser "Plug and Play" en cualquier equipo moderno. Puesto que la Base de Datos (`diodev_minimarlet`) ya está viva en la red, no requieres configurar XAMPP ni MySQL en tu máquina local.

**Requisitos Previos:**
1. Tener instalado el **Java Development Kit (JDK 8 o superior)**.
2. Contar con una carpeta funcional del servidor **Apache Tomcat 9.x**.

**Pasos para el Despliegue:**
1. Toma el archivo comprimido final del proyecto: `UrbanMinimarket.war`.
2. Pega este archivo `.war` dentro de la subcarpeta `webapps` que se encuentra en la ruta de tu Apache Tomcat (Ej: `apache-tomcat-9.0.x/webapps/`).
3. Inicia el servidor ejecutando el script `startup.bat` (Windows) o `startup.sh` (Mac/Linux) ubicado en la carpeta `bin/` de Tomcat.
4. El servidor desempaquetará el proyecto de manera automática.

Accede al entorno visitando la dirección en cualquier navegador:
 **[http://localhost:8080/UrbanMinimarket/](http://localhost:8080/UrbanMinimarket/)**
...
- **Administrador del Sistema:** 
  - Correo: `admin@urbanminimarket.com`
  - Contraseña: `admin123`
  *(Permite la gestión total de inventarios, pedidos y clientes).*
  
- **Cliente Frecuente:**
  - Correo: `cliente@urbanminimarket.com`
